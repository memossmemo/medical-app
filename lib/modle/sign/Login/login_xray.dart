...
class Login_xray extends StatefulWidget {
  const Login_xray({super.key});

  @override
  State<Login_xray> createState() => _LoginState();
}

class _LoginState extends State<Login_xray> {
  ...
  Future<void> signinPharmatic() async {
    // The URL of your API
    final url = Uri.parse('https://pharma-manager-copy-2.onrender.com/api/Radiology/signinRadiology');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        shared?.setString("token_radio", data['token']);
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Center(
              child: Container( margin: EdgeInsets.only(top: 80),width: 300,
                height: 50,
                child: Custom_Text(
                  validator: (val){
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                foregroundColor: Colors.white, backgroundColor: Colors.blue, // لون النص عند الضغط
                padding: EdgeInsets.symmetric(vertical: 12.0, horizontal: 25.0), // المسافة حول النص
